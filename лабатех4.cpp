...
class Inventory {
public:
    Inventory(const std::string& filename) : filename(filename) {
        loadFromFile();
    }
        ...
        if (file.is_open()) {
            ...
            while (std::getline(file, line)) {
                ...
                if (std::getline(iss, id, ',') &&
                    ...
                    products.emplace_back(id, name, price, category, description);
                }
            }
            file.close();
        }
    ...
    while (true) {
        ...
        if (choice == "1") {
            ...
            if (product) {
                ...
                inventory.editProduct(id, product->name, product->price, product->category, product->description);
            ...
            Product* product = inventory.findProduct(id);
            if (product) {
                inventory.displayProduct(*product);
            }
            else {
                std::cout << "Product not found.\n";
            }
